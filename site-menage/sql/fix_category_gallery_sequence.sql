-- ============================================
-- Corriger la séquence de la clé primaire pour category_gallery
-- Fix primary key sequence for category_gallery table
-- ============================================

-- VERSION RAPIDE - Exécutez simplement cette ligne:
-- QUICK VERSION - Just execute this line:
SELECT setval('category_gallery_id_seq', COALESCE((SELECT MAX(id) FROM category_gallery), 0) + 1, false);
...
SELECT 
  ...
  (SELECT last_value FROM category_gallery_id_seq) AS current_sequence_value
...
SELECT setval(
  'category_gallery_id_seq',
  COALESCE((SELECT MAX(id) FROM category_gallery), 0) + 1,
  false
...
SELECT 
  ...
  (SELECT MAX(id) FROM category_gallery) AS max_id_in_table,
  CASE 
    WHEN last_value > (SELECT MAX(id) FROM category_gallery) THEN '✅ Séquence corrigée'
    ELSE '⚠️ Vérifiez manuellement'
  END AS status
FROM category_gallery_id_seq;
...
--    SELECT pg_get_serial_sequence('category_gallery', 'id');
