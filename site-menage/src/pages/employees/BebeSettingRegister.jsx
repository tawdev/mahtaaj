...
export default function BebeSettingRegister() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      fd.append('auto_entrepreneur', form.auto_entrepreneur || '');
      fd.append('last_experience', form.last_experience || '');
      fd.append('company_name', form.company_name || '');
      if (form.preferred_work_time) fd.append('preferred_work_time', form.preferred_work_time);
      fd.append('availability', JSON.stringify(selectedDaysPayload));
      if (form.photo instanceof File) fd.append('photo', form.photo);
      
      const res = await fetch(`${API_BASE_URL}/api/employees/bebe`, {
        method: 'POST',
        body: fd
      });
      ...
      if (!res.ok || !data.success) {
        throw new Error(data.message || t('employees.register.submit_failed','فشل في إرسال النموذج'));
      }
