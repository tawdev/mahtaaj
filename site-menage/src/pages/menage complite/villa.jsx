...
export default function Villa() {
  ...
  const [reservationData, setReservationData] = useState({}); // { itemId: { floors: 0, rooms: [], bathrooms: [], salons: [], hasGarden: false, garden: { length: '0', width: '0' }, hasPool: false, pool: { length: '0', width: '0' } } }
  ...
  useEffect(() => {
    const loadVilla = async () => {
      try {
        ...
        console.error('[Villa] Exception loading data:', err);
  ...
  // Calculate price for a single room/bathroom/salon/pool
  ...
  const handleReserve = (item) => {
    ...
    if (finalPrice === 0) {
      alert(t('villa_page.please_fill_form', 'Veuillez remplir le formulaire de réservation'));
      return;
    }
      ...
      <main className="menage-complite-page">
        <div className="menage-complite-header">
          <button 
            ...
            title={t('villa_page.back', 'Retour')}
          >
            ← {t('villa_page.back', 'Retour')}
          </button>
        </div>
        <div className="menage-complite-loading">Chargement des services Villa...</div>
      </main>
    );
  }
  ...
  if (!items.length) {
    return (
      <main className="menage-complite-page">
        ...
        <div className="menage-complite-empty">Aucun service Villa disponible pour le moment.</div>
      </main>
    );
  }
          ...
          return (
            <article key={item.id} className="menage-complite-card">
              ...
              <div className="menage-complite-card-body">
                ...
                {/* Reservation Form */}
                <div className="villa-reservation-form">
                  {/* Floors */}
                  <div className="villa-form-group">
                    <label className="villa-form-label">
                      {t('villa_page.floors', 'عدد الطوابق')}:
                    </label>
                    <input
                      type="number"
                      min="0"
                  ...
                  <div className="villa-form-group">
                    <label className="villa-form-label">
                      {t('villa_page.rooms_count', 'عدد الغرف')}:
                    </label>
                    <input
                      type="number"
                      min="0"
                    ...
                    {(reservationData[item.id]?.rooms || []).length > 0 && (
                      <div className="villa-rooms-list">
                        {(reservationData[item.id]?.rooms || []).map((room, index) => {
                          ...
                          const price = calculateRoomPrice(room.length, room.width);
                          return (
                            <div key={index} className="villa-room-item">
                              <div className="villa-room-header">
                                <span className="villa-room-title">
                                  {t('villa_page.room', 'غرفة')} {index + 1}
                                </span>
                                {price > 0 && (
                                  <span className="villa-room-price">
                                    {price.toFixed(2)} DH ({area.toFixed(2)} m²)
                                  </span>
                                )}
                              </div>
                              <div className="villa-dimensions">
                                <div className="villa-dimension-group">
                                  <label className="villa-dimension-label">
                                    {t('villa_page.length', 'الطول (cm)')}:
                                  </label>
                                  <input
                                    type="text"
                                    inputMode="decimal"
                                    ...
                                    className="villa-dimension-input"
                                    placeholder="0"
                                  />
                                </div>
                                <div className="villa-dimension-group">
                                  <label className="villa-dimension-label">
                                    {t('villa_page.width', 'العرض (cm)')}:
                                  </label>
                                  <input
                                    type="text"
                                    inputMode="decimal"
                  ...
                  <div className="villa-form-group">
                    <label className="villa-form-label">
                      {t('villa_page.bathrooms_count', 'عدد الحمامات')}:
                    </label>
                    <input
                      type="number"
                      min="0"
                    ...
                    {(reservationData[item.id]?.bathrooms || []).length > 0 && (
                      <div className="villa-bathrooms-list">
                        {(reservationData[item.id]?.bathrooms || []).map((bathroom, index) => {
                          const area = calculateArea(bathroom.length, bathroom.width);
                          const price = calculateRoomPrice(bathroom.length, bathroom.width);
                          return (
                            <div key={index} className="villa-bathroom-item">
                              <div className="villa-bathroom-header">
                                <span className="villa-bathroom-title">
                                  {t('villa_page.bathroom', 'حمام')} {index + 1}
                                </span>
                                {price > 0 && (
                                  <span className="villa-bathroom-price">
                                    {price.toFixed(2)} DH ({area.toFixed(2)} m²)
                                  </span>
                                )}
                              </div>
                  ...
                  <div className="villa-form-group">
                    <label className="villa-form-label">
                      {t('villa_page.salons_count', 'عدد الصالونات')}:
                    </label>
                    <input
                      type="number"
                      min="0"
                    ...
                    {(reservationData[item.id]?.salons || []).length > 0 && (
                      <div className="villa-salons-list">
                        {(reservationData[item.id]?.salons || []).map((salon, index) => {
                          const area = calculateArea(salon.length, salon.width);
                          const price = calculateRoomPrice(salon.length, salon.width);
                          return (
                            <div key={index} className="villa-salon-item">
                              <div className="villa-salon-header">
                                <span className="villa-salon-title">
                                  {t('villa_page.salon', 'صالون')} {index + 1}
                                </span>
                                {price > 0 && (
                                  <span className="villa-salon-price">
                                    {price.toFixed(2)} DH ({area.toFixed(2)} m²)
                                  </span>
                                )}
                              </div>
                  ...
                  {/* Garden */}
                  <div className="villa-form-group">
                    <label className="villa-checkbox-label">
                      <input
                        type="checkbox"
                        checked={reservationData[item.id]?.hasGarden || false}
                        onChange={() => handleGardenToggle(item.id)}
                        className="villa-checkbox"
                      />
                      <span>{t('villa_page.has_garden', 'هل توجد حديقة؟')}</span>
                    </label>
                    {reservationData[item.id]?.hasGarden && (
                      <div className="villa-garden-section">
                        <div className="villa-garden-header">
                          <span className="villa-garden-title">
                            {t('villa_page.garden', 'الحديقة')}:
                          </span>
                        </div>
                        <div className="villa-dimensions">
                          <div className="villa-dimension-group">
                        ...
                        {reservationData[item.id]?.garden && (() => {
                          ...
                          return price > 0 ? (
                            <div className="villa-garden-price">
                              {price.toFixed(2)} DH ({area.toFixed(2)} m²)
                            </div>
                          ) : null;
                        })()}
                  ...
                  <div className="villa-form-group">
                    <label className="villa-checkbox-label">
                      <input
                        ...
                        className="villa-checkbox"
                      />
                      <span>{t('villa_page.pool_cleaning', 'هل تريد تنظيف المسبح؟')}</span>
                    </label>
                    {reservationData[item.id]?.hasPool && (
                      <div className="villa-pool-section">
                        <div className="villa-pool-header">
                          <span className="villa-pool-title">
                            {t('villa_page.pool', 'المسبح')}:
                          </span>
                        </div>
                        <div className="villa-dimensions">
                          <div className="villa-dimension-group">
                        ...
                        {reservationData[item.id]?.pool && (() => {
                          ...
                          return price > 0 ? (
                            <div className="villa-pool-price">
                              {price.toFixed(2)} DH ({area.toFixed(2)} m²)
                            </div>
                          ) : null;
                        })()}
                  ...
                  {finalPrices[item.id] > 0 && (
                    <div className="villa-final-price">
                      <span className="villa-final-price-label">
                        {t('villa_page.total_price', 'السعر الإجمالي')}:
                      </span>
                      <span className="villa-final-price-value">
                ...
                <button
                  className={`villa-card-reserve-btn ${finalPrices[item.id] <= 0 ? 'villa-card-reserve-btn-disabled' : ''}`}
                  onClick={() => handleReserve(item)}
                  disabled={finalPrices[item.id] <= 0}
                >
                  {t('villa_page.reserve', 'Réserver')}
                </button>
              </div>
            </article>
          );
