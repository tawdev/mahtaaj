  ...
  const handleAutoLocate = async () => {
    try {
      if (!navigator.geolocation) {
        alert(t('checkout.geo_not_supported') || 'Géolocalisation non supportée par votre navigateur.');
        return;
      }
      await new Promise((resolve, reject) => {
        ...
        // Do not block the UI with setError, just alert the user or fail silently so they can type
        alert(t('checkout.geo_unavailable') || 'Impossible de récupérer la localisation.');
      });
    } catch (e) {
      console.error('Geolocation exception:', e);
      alert(t('checkout.geo_error') || 'Erreur de géolocalisation.');
    }
  };
