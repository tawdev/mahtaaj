...
export default function ReservationBureuxUsin() {
  ...
  const [initialData, setInitialData] = useState(() => {
    ...
    if (location.state?.type) {
      return {
        ...
        serviceType: location.state.serviceType || 'bureaux'
      };
    }
    // 2. Try sessionStorage
    const saved = sessionStorage.getItem('bureaux_usin_pending_reservation_state');
    if (saved) {
      try {
  ...
  const serviceType = initialData?.serviceType || 'bureaux';
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        if (!session) {
          console.log('[ReservationBureuxUsin] No session found, redirecting to login...');
          // Save state to survive redirect
          if (location.state) {
            sessionStorage.setItem('bureaux_usin_pending_reservation_state', JSON.stringify(location.state));
          }
          localStorage.setItem('auth_return_url', location.pathname + location.search);
          console.log('[ReservationBureuxUsin] Saved auth_return_url:', location.pathname + location.search);
        ...
        sessionStorage.removeItem('bureaux_usin_pending_reservation_state');
  ...
  if (isCheckingAuth) {
    return <main className="reservation-bureaux-usin-page"><div className="reservation-bureaux-usin-loading">Vérification de l'authentification...</div></main>;
  }
