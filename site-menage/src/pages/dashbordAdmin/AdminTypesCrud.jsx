...
export default function AdminTypesCrud({ token, onAuthError }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare type data - explicitly exclude id, created_at, updated_at for new records
      const typeData = {
        category_house_id: formData.category_house_id || null,
      ...
      delete typeData.id;
      delete typeData.created_at;
      delete typeData.updated_at;
      ...
      if (editingType) {
        ...
        // Create new type - use single() to get one result instead of array
        const result = await supabase
          .from('types')
          .insert(typeData)
          .select('id, category_house_id, name, name_ar, name_fr, name_en, description, description_ar, description_fr, description_en, image, price, is_active, order, created_at, updated_at')
      ...
      if (error) {
        ...
        if (error.code === '23505') {
          if (error.message?.includes('pkey') || error.message?.includes('primary key')) {
            ...
            errorMessage += 'Veuillez exécuter le script SQL: sql/fix_types_sequence.sql dans Supabase SQL Editor pour corriger ce problème.';
          ...
          errorMessage += 'Erreur de référence. Vérifiez les relations (category_house_id).';
  ...
  const handleOptionSubmit = async (e) => {
    ...
    try {
      ...
      // Prepare option data - explicitly exclude id, created_at, updated_at for new records
      const optionData = {
        type_id: selectedType?.id || null,
        name: optionFormData.name?.trim() || null,
        name_ar: optionFormData.name_ar?.trim() || null,
        name_fr: optionFormData.name_fr?.trim() || null,
        name_en: optionFormData.name_en?.trim() || null,
        description: optionFormData.description?.trim() || null,
        description_ar: optionFormData.description_ar?.trim() || null,
        description_fr: optionFormData.description_fr?.trim() || null,
        description_en: optionFormData.description_en?.trim() || null,
        image: imageUrl || optionFormData.image || null,
        price: parseFloat(optionFormData.price) || 0,
        is_active: optionFormData.is_active !== undefined ? optionFormData.is_active : true,
        order: parseInt(optionFormData.order) || 0
      };
      
      // Ensure we never send id, created_at, or updated_at for new records
      delete optionData.id;
      delete optionData.created_at;
      delete optionData.updated_at;
      ...
      if (editingOption) {
        ...
        const result = await supabase
          ...
          .select('id, type_id, name, name_ar, name_fr, name_en, description, description_ar, description_fr, description_en, image, price, is_active, order, created_at, updated_at');
        data = result.data && result.data.length > 0 ? result.data[0] : null;
        error = result.error;
      } else {
        // Create new option - use single() to get one result instead of array
        const result = await supabase
          .from('type_options')
          .insert(optionData)
          .select('id, type_id, name, name_ar, name_fr, name_en, description, description_ar, description_fr, description_en, image, price, is_active, order, created_at, updated_at')
      ...
      if (error) {
        ...
        if (error.code === '23505') {
          if (error.message?.includes('pkey') || error.message?.includes('primary key')) {
            errorMessage += 'Erreur de clé primaire: la séquence de la base de données est désynchronisée. ';
            errorMessage += 'Veuillez exécuter le script SQL approprié dans Supabase SQL Editor pour corriger ce problème.';
          } else if (error.message?.includes('unique')) {
            errorMessage += 'Une valeur unique existe déjà. Veuillez utiliser une valeur différente.';
          } else {
            errorMessage += 'Une valeur unique existe déjà. Veuillez utiliser une valeur différente.';
          ...
          errorMessage += 'Erreur de référence. Vérifiez les relations (type_id).';
        } else if (error.code === '23514' || error.message?.includes('check constraint')) {
          errorMessage += 'Les données ne respectent pas les contraintes de validation.';
        } else if (error.code === 'PGRST116' || error.message?.includes('permission') || error.message?.includes('policy')) {
          errorMessage += 'Erreur de permissions. Vérifiez les politiques RLS.';
